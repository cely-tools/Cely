...
class StorageTests: XCTestCase {
    ...
    func testSavingData() {
        dummyData.forEach { dummy in
            ...
            if dummy.value != nil {
                let successStatus = success == StorageResult.success || success == StorageResult.fail(.duplicateItem)
                XCTAssert(successStatus, dummy.failedToSet())
            } else {
                XCTAssert(StorageResult.fail(.unexpectedError) == success, "You're not supposed to be able to set nil in the storage.")
            }
        }
    }
